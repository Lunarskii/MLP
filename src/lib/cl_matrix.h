...
class Matrix {
    ...
    cl::Buffer matrix_;
    public:
        Matrix(size_t row, size_t col) : rows_(row), cols_(col),
        ...
        Matrix(size_t row, size_t col, const std::vector<T> &vec) : Matrix(row, col) {
            ...
            queue.enqueueWriteBuffer(matrix_, CL_TRUE, 0, sizeof(T) * row * col, vec.data());
        }
        ...
        Matrix(std::initializer_list<std::initializer_list<T>> const &items) :
            ...
            queue.enqueueWriteBuffer(matrix_, CL_TRUE, 0, sizeof(T) * rows_ * cols_, buffer.data());
        }

        void Update(const std::vector<T> &vec) {
            cl::CommandQueue queue(context_, default_device_);
            queue.enqueueWriteBuffer(matrix_, CL_TRUE, 0, sizeof(T) * rows_ * cols_, vec.data());
        }
        ...
        void Set(size_t row, size_t col, T value) {
            cl::CommandQueue queue(context_, default_device_);
            queue.enqueueWriteBuffer(matrix_, CL_TRUE, sizeof(T) * (row * cols_ + col), sizeof(T), &value);
        }
        T Get(size_t row, size_t col) const {
            cl::CommandQueue queue(context_, default_device_);
            T element;
            queue.enqueueReadBuffer(matrix_, CL_TRUE, sizeof(T) * (row * cols_ + col), sizeof(T), &element);
            return element;
        }
...
class Arithmetic {
    public:
        ...
        static void Mul(const Matrix<T> &A, const Matrix<T> &B, Matrix<T> &C, const T alpha = 1.0, const double beta = 0.0) {
            ...
            // clblasSetup();
            functions<T>::mul(clblasRowMajor, clblasNoTrans, clblasNoTrans, A.rows_, B.cols_, A.cols_,
                        alpha, A.matrix_(), 0, A.cols_, B.matrix_(), 0, B.cols_, beta, C.matrix_(), 0, C.cols_, 1,
